...
describe('DebugPane (Results)', () => {
  ...
  it('redacts the Authorization header in the request panels', async () => {
    ...
    useWorkflowStore.setState({
      nodes: [{ id: 'node-1', kind: 'operation', operationId: 'POST /pet', credentialId: null }],
      operations: [petOp],
      runResult: {
        steps: [
          makeStep({
  ...
  it('summarizes each step with the canvas node label, including #N for duplicates', () => {
    useWorkflowStore.setState({
      nodes: [
        { id: 'node-1', kind: 'operation', operationId: 'POST /pet', credentialId: null },
        { id: 'node-2', kind: 'operation', operationId: 'POST /pet', credentialId: null },
      ],
      operations: [petOp],
  ...
  it('shows a pause bar with Step targeting the paused node', async () => {
    ...
    useWorkflowStore.setState({
      nodes: [{ id: 'a', kind: 'operation', operationId: 'GET /a', credentialId: null }],
      connections: [],
      operations: [
  ...
  it('lists every node with live status once the canvas has steps', () => {
    useWorkflowStore.setState({
      nodes: [
        { id: 'a', kind: 'operation', operationId: 'GET /a', credentialId: null },
        { id: 'b', kind: 'operation', operationId: 'GET /b', credentialId: null },
      ],
